...
## Pwnbox Encode SSH Key to Base64
```shell
f3nr1r@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```shell
f3nr1r@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell
f3nr1r@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell
f3nr1r@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell
f3nr1r@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell
f3nr1r@htb[/htb]$ echo  | base64 -d > hosts
```
...
```shell
f3nr1r@htb[/htb]$ pip3 install uploadserver
...
f3nr1r@htb[/htb]$ python3 -m uploadserver
...
```shell
f3nr1r@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
